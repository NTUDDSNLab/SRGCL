...
def calculate_distance(data_batch1, data_batch2, anchor_model, selector):    
    anchor_x = anchor_model.forward_cl(data_batch1[1], data_batch1[0])
    aug_x = anchor_model.forward_cl(data_batch2[1], data_batch2[0])
    with torch.no_grad():
        if(selector == 'cosine'):
...
def train_cl_with_sim_loss_temperature(view_gen1, view_gen2, view_optimizer, model, anchor_model, optimizer, 
    ...
    for data in data_loader:
        ...
        for _ in range(generated_views_num):
            ...
            distance = calculate_distance(input_pair_list[0], input_pair_list[1], anchor_model, selector)
            distances1.append((distance, view1, sample1))

            distance = calculate_distance(input_pair_list[0], input_pair_list[2], anchor_model, selector)
        ...
        out1 = model.forward_cl(closest_data_batch_augmentations1[0], closest_sample_augmentations1[0])
        out2 = model.forward_cl(closest_data_batch_augmentations2[0], closest_sample_augmentations2[0])
...
if __name__ == "__main__":
    ...
    anchor_model = graphcl(gnn, dataset)
    model.to(device)
    ...
    for epoch in range(1, args.epochs+1):    
        ...
        train_loss = train_cl_with_sim_loss_temperature(
            ...
            optimizer, train_loader, device, selector, epoch, 
