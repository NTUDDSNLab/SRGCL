...
def arg_parse():
    ...
    parser.add_argument('--save', type=str, default = 'with_sim_loss', help='debug')
...
def cl_exp(args):
    ...
    while True:
        ...
        for data in select_dataloader:
            ...
            # print("data_ori  ", data_ori)                                      # data_ori   DataBatch(edge_index=[2, 5116], x=[2311, 109], y=[128], batch=[2311], ptr=[129])
            # print("data  ", data)                                              # data   DataBatch(edge_index=[2, 3164], x=[2311, 109], y=[128], batch=[2311], ptr=[129])
                                                                               # data_ori   DataBatch(edge_index=[2, 2326], x=[1060, 109], y=[60], batch=[1060], ptr=[61])
                                                                               # data   DataBatch(edge_index=[2, 1526], x=[1060, 109], y=[60], batch=[1060], ptr=[61])
...
def graph_cl_exp(args):
    ...
    for epoch in range(1, epochs+1):
        ...
        for data in second_dataloader:
            ...
            # if args.aug == 'dnodes' or args.aug == 'subgraph' or args.aug == 'random2' or args.aug == 'random3' or args.aug == 'random4':
            #     # node_num_aug, _ = data_aug.x.size()
            #     edge_idx = data_aug.cpu().edge_index.numpy()
            #     _, edge_num = edge_idx.shape
            #     idx_not_missing = [n for n in range(node_num) if (n in edge_idx[0] or n in edge_idx[1])]
            ...
            loss = second_model.second_loss_cal(x, x_aug, indices_list)
            # logger.info(loss.item())
            loss_all += loss.item() * data.num_graphs
            total_graphs += data.num_graphs
            loss.backward()
