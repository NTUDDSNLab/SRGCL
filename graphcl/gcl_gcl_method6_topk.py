...
def generate_aug_data_batch(data_batch, anchor_model, topk_views_cl=2, generated_views_num=50, augmentation_type='dnodes'):
    ...
    for graph in data_batch.to_data_list():        
        ...
        # print(distances)
        for i in range(min(topk_views_cl, len(distances))):
            grouped_aug_graphs[i].append(distances[i][1])
    data_batches = []
...
if __name__ == '__main__':
    ...
    for epoch in range(1, epochs+1):
        ...
        for data in dataloader:
            ...
            grouped_aug_graphs_batches = generate_aug_data_batch(data, anchor_model, topk_views_cl, generated_views_num, augmentation_type)
            x_aug = []
            for i in range(len(grouped_aug_graphs_batches)):
                x_aug.append(model(grouped_aug_graphs_batches[i].x, grouped_aug_graphs_batches[i].edge_index, grouped_aug_graphs_batches[i].batch, grouped_aug_graphs_batches[i].num_graphs))
