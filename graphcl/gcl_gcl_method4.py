...
def generate_data_batch_augmentation(data_batch, generated_views_num=10, augmentation_type='dnodes'):
    aug_ratio = 0.2
    augmentations = []
...
if __name__ == '__main__':
    ...
    for epoch in range(1, epochs+1):
        ...
        for data in dataloader:
            ...
            for aug in closest_data_batch_augmentations:
                good_aug_data_list.append(aug)
            
            x_aug = []
            for i in range(len(closest_data_batch_augmentations)):
                x_aug.append(model(closest_data_batch_augmentations[i].x, closest_data_batch_augmentations[i].edge_index, closest_data_batch_augmentations[i].batch, closest_data_batch_augmentations[i].num_graphs))
