...
if __name__ == '__main__':
    ...
    for epoch in range(1, epochs+1):
        ...
        for data in dataloader:
            ...
            total_batches += 1
            data, _ = data
            data_aug = get_augmentation(data, aug=args.aug)
            data = data.to(device)
            data_aug = data_aug.to(device)

            # Anchor model calculate distance
            anchor_x = anchor_model(data.x, data.edge_index, data.batch, data.num_graphs)
            aug_x = anchor_model(data_aug.x, data_aug.edge_index, data_aug.batch, data_aug.num_graphs)
            if(selector == 'cosine'):
            ...
            distance_means.append(distances.mean().item())

            threshold = np.mean(distance_means) + 0.5 * np.std(distance_means)
            ...
            loss.backward()
            optimizer.step()
        print(f"Skipped {skipped_batches}/{total_batches} batches.")
