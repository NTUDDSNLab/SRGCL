...
if __name__ == '__main__':
    ...
    for epoch in range(1, epochs+1):
        ...
        for data in second_dataloader:
            ...
            node_num, _ = data.x.size()
            data = data.to(device)
            x = second_model(data.x, data.edge_index, data.batch, data.num_graphs)
            ...
            x_aug = second_model(data_aug.x, data_aug.edge_index, data_aug.batch, data_aug.num_graphs)

            # print(x)
            # print(x_aug)
            loss_aug = model.second_loss_cal(x, x_aug, indices_list)
            loss = loss_aug
            # print(loss)
            loss_all += loss.item() * data.num_graphs
            loss.backward()
            optimizer.step()
            # print('batch')
        # print('Epoch {}, Loss {}'.format(epoch, loss_all / len(second_dataloader)))
    ...
    with open('logs/log_' + args.DS + '_' + args.aug, 'a+') as f:
        ...
        f.write('{},{},{},{},{},{},{},{},{},{},{},{},{}\n'.format(args.DS, tpe, args.num_gc_layers, epochs, log_interval, lr, aug_data_ratio, dataset_len_multiple, s1, s2, s3))
        f.write('\n')
